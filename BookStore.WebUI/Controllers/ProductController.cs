...
namespace BookStore.WebUI.Controllers
{
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> CreateProduct(CreateProductDto createProductDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7287/api/Products", content);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7287/api/Products/GetProduct?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateProduct(UpdateProductDto updateProductDto)
        {
            ...
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7287/api/Products", content);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("ProductList");
            }
